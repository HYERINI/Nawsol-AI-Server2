...
class ETFRecommendationService:
    ...
    @staticmethod
    def _build_etf_list(etf_data: List[Dict]) -> str:
        """ETF 목록 문자열 생성"""
        if not etf_data:
            return "ETF 데이터가 없습니다."
        
        etf_parts = []
        etf_parts.append(f"📈 분석 가능한 ETF 목록 ({len(etf_data)}개)")
        
        # 상위 10개 ETF만 표시 (시가총액 기준)
        sorted_etfs = sorted(
            etf_data,
            key=lambda x: x.get('mrktTotAmt', 0) or 0,
            reverse=True
        )[:10]
        
        for idx, etf in enumerate(sorted_etfs, 1):
            name = etf.get('bssIdxIdxNm', 'N/A')
            price = etf.get('clpr', 0)
            change_rate = etf.get('fltRt', 0)
            market_cap = etf.get('mrktTotAmt', 0)
            
            etf_parts.append(
                f"{idx}. {name} | "
                f"가격: {price:,}원 | "
                f"등락률: {change_rate:+.2f}% | "
                f"시총: {market_cap/100000000:,.0f}억원" if market_cap else f"시총: N/A"
            )
    ...
    async def recommend_etf(
        ...
        try:
            ...
            # ETF 목록 생성
            etf_list = cls._build_etf_list(etf_data)
            
            # AI 프롬프트 작성 (1부)
            prompt_part1 = f"""당신은 전문 재무 상담사입니다. 사용자의 재무 상황을 분석하고 적합한 ETF를 추천해주세요.
